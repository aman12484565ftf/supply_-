...
import { X, Package, DollarSign, Tag, BarChart2 } from "lucide-react";
...
const AddProductModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Log formData to check if it's complete
    console.log("🆕 Form Data Before Sending:", formData);
  
    const newProduct = {
      name: formData.name,
      price: parseFloat(formData.price), // Ensure it's a number
      stock: parseInt(formData.stock),   // Ensure it's a number
    ...
    try {
      ...
      await dispatch(createProduct(newProduct)); // ✅ Dispatch Redux action
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-8 flex gap-3">
            ...
            <button 
              type="submit" 
              className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-300 transform hover:-translate-y-1"
            >
              Add Product
            </button>
          </div>
